...
from dataset.data_uad import get_loader as get_loader
...
def main(config, seed):
    ...
    train_data_loader = get_loader(
        ...
        # num_workers=0, ## 디버깅하려면 0으로 설정
        aug4imgRatio=config["aug4imgRatio"]
    )
    val_data_loader = get_loader(
        batch_size=config["batch_size"],
        img_size=config["img_size"],
        split_file="Unseen", ## shlee changed
        data_dir=config["test_data_dir"], ## shlee changed
        shuffle=False,
        train=False,
        exo_obj_file=None, 
        ego_obj_file=None, 
