...
class ModelAGDsup(nn.Module):
    ...
    def __init__(self, 
        ...
        self.active_transform = nn.Sequential(
            nn.Linear(512, 512),
            nn.GELU(),
            nn.Linear(512, 512)
        )
        self.passive_transform = nn.Sequential(
            nn.Linear(512, 512),
            nn.GELU(),
            nn.Linear(512, 512)
        )
    ...
    def forward(self, imgs, text_feat, exo=None, exo_obj_mask=None, num_exo=1,active_f = None,passive_f = None):
        ...
        # pred_part=None ## for wo part loss...

        
        ## v1(active와 passive로 예측)
        ...
        #     aff_token, _, _ = self.verb_fuser(x, pred_part+v+pred_context_aware)
        #     # aff_token, _, _ = self.verb_fuser(x, v+pred_context_aware) ## wo part
        ## v2
        if passive_f !=None and active_f != None:
            passive_feature = passive_f.float().unsqueeze(1)
            active_feature = active_f.float().unsqueeze(1)
        ...
        if exo is not None:
            ...
            if passive_f !=None and active_f != None:
                return pred_heatmap, sim_loss, self.exo_cls(exo_token), pred_noun, pred_part, pred_context_aware 
            else: 
