...
from dataset.data_context_aware import get_loader as get_loader
...
def main(config, seed):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_data in tqdm(train_data_loader):
            ...
            if config["num_exo"] > 0:
                aff_res, sim_loss, exo_cls_res, pred_noun, pred_part, context_aware = model(
                    batch_data["input_image"], batch_data["sent_feats"], 
                    batch_data["exo_image"], 
                    batch_data["exo_objbox_mask_patch"], num_exo=config["num_exo"],
                    active_f = batch_data["active_feats"],passive_f = batch_data["passive_feats"],
                )
            else:
                aff_res, pred_noun, pred_part, context_aware = model(
                    batch_data["input_image"], batch_data["sent_feats"], 
                    active_f = batch_data["active_feats"],passive_f = batch_data["passive_feats"],
                )
            ...
            context_sim_loss = (1 - F.cosine_similarity(context_aware, batch_data["sent_feats"].cuda(),dim=2)).mean() ## part-active 정보로부터 추정된 결과는 affordance verb가 되도록..
                ...
                loss_config["sim_loss_coeff"] * sim_loss + \
                    loss_config["exo_cls_coeff"] * exo_cls_loss + \
                        loss_config["noun_sim_coeff"] * noun_sim_loss + \
                            loss_config["context_sim_coeff"] * context_sim_loss + \
            ...
            all_context_sim_loss += context_sim_loss.detach().item()
        ...
        logger.info(
            ...
            f"Context_simm_loss: {all_context_sim_loss / all_num}")
        logger.info(
        ...
        with torch.no_grad():
            ...
            for batch_data in tqdm(val_data_loader):
                aff_res, pred_noun, pred_part,pred_context = model(
                    batch_data["input_image"], batch_data["sent_feats"],
                    active_f = batch_data["active_feats"],passive_f = batch_data["passive_feats"],
                )
                pred = aff_res.detach()
