...
def main(config):
    ### --------------------------------------------------------------------------------------------------------------------------------
    # pred_dir="../../04_UAD/unsup-affordance/logs/20250721/oai_behavior1k/ckpts/aff-obj_unsee_best/"
    pred_dir="../../04_UAD/unsup-affordance/checkpoints/oai_emb/"
    ### --------------------------------------------------------------------------------------------------------------------------------
    ...
    with torch.no_grad():
        for batch_data in tqdm(eval_data_loader):
            pred_path = batch_data['input_paths'][0].replace("../../AGD20K/Unseen/testset/egocentric/",pred_dir).replace(".jpg",".png")
            if not os.path.exists(pred_path):
                import pdb;pdb.set_trace()
                print("check pathes!!")
            pred = cv2.imread(pred_path, cv2.IMREAD_GRAYSCALE)
            # pred = cv2.imread(pred_path)
            pred=torch.Tensor(pred/255.).unsqueeze(0).unsqueeze(0)
            ...
            r_prob = F.softmax(r_pred.reshape(len(pred), -1), dim=1).to("cuda:0")
            r_prob = r_prob / (r_prob.sum(dim=1, keepdim=True) + 1e-12)
            # import pdb;pdb.set_trace()
            ...
            for bid in range(len(r_pred)):
                ...
                ## visualize with gt
                # import pdb;pdb.set_trace()
                plot_annotation_with_gt(eval_data_loader.dataset.get_input_image_back(ii), ## (224, 224, 3)
                                pp.reshape(*batch_data["gt_mask"][bid].shape).detach().cpu().numpy().transpose(1, 2, 0), ## (224, 224, 1)
                                batch_data["gt_mask"][bid].cpu().numpy().transpose(1, 2, 0), ## (224, 224, 1)
                                name=config['work_dir']+f'/img/AGD{vall_num}_{bid}_{kld_per_sample[bid]}_{verbs[bid]}pred.png')
